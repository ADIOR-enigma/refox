import { IPalette, IPaletteTemplate, IPywalColors } from "@definitions";

import Generators from "@refox/background/generators";
import {
  EXTENDED_PYWAL_COLORS,
  DEFAULT_THEME_DARK,
...
const defaultPalette: IPalette = Generators.palette(
  pywalPalette,
  {},
  paletteTemplate,
);
...
describe("generatePywalPalette", () => {
  test("creates an extended pywalColors array", () => {
    expect(pywalPalette.length).toBe(
      pywalColors.length + EXTENDED_PYWAL_COLORS.length,
    );
  });
});
...
describe("generatePalette", () => {
  ...
  test("custom colors override generated palette", () => {
    const palette: IPalette = Generators.palette(
      pywalColors,
      customColors,
      paletteTemplate,
    );
...
describe("generateWebsiteTheme", () => {
  ...
  test("creates namespaced CSS variables for websites", () => {
    ...
    expect(theme).toContain("--refox-background:");
    expect(theme).toContain("--refox-background-light:");
    expect(theme).toContain("--refox-accent-primary:");
  });
