...
2. Install the native messaging host using one of the two methods below:

   ### Method 1: Automated Installation (Arch Linux / Arch-based)
   Run `sudo install.sh` to automate the install [works on Firefox, Librewolf, Floorp, Mercury, Zen].
   ...
   3. **Match manifest with Re:fox:** Because Re:fox is forked from Pywalfox, edit the generated manifest file (`~/.mozilla/native-messaging-hosts/pywalfox.json` on Linux/macOS or system/fork/Windows equivalent) and add `"re-fox@adior.org"` to `allowed_extensions`:
      ```json
      {
        "name": "pywalfox",
...
Because Zen uses unique CSS custom properties (such as `--zen-accent-primary` and `--zen-background`), we provide experimental `userChromeJS` support inside the [zen/](./zen) folder of this repository:
